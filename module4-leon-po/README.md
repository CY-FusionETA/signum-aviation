...
Point the vhost **DocumentRoot at `public/`** (config/, src/, db/, storage/ sit above the
web root). `public/.htaccess` does the front-controller routing; under a subpath set
`RewriteBase` and `app.base_url` to match.
